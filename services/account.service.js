...
async function directPasswordReset(param) {
  const resetTokenRes = await forgotPassword(param);
  const resetPasswordRes = await resetPassword({
    token: resetTokenRes.account.resetToken,
    password: param.password,
  });
  if (resetTokenRes.account.verificationToken) {
    verifyEmail({ token: resetTokenRes.account.verificationToken });
  }
...
async function createPosition(req) {
  const {
    positiontitle,
    projectId,
    experience,
